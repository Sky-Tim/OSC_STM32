/*---------------------------------------------------------------------------------
	    				STM32 SD卡驱动程序(SDIO模式)
 说明: 本SD驱动程序由ST公司提供的库修改而来,经过测试程序运行基本稳定,无不良现象.
 	   本SD驱动只是裸机驱动,没有提供文件系统,没有运行操作系统.目前这个版本的驱动
	   只能在STM32上使用,SPI模式可以在51单片机上使用.本驱动程序使用了DMA2通道4来
	   加速数据的传输.

 使用方法: SD驱动程序配置步骤
           1) 在工程中添加驱动程序和头文件
		   2) 在SD_GPIO_Configuration()中配置硬件引脚和使能时钟
		   3) 在SD_NVIC_Configuration()中配置SD中断
		   4) 在SDIO_IRQHandler()中配置中断服务程序
		   5) 在SD_DMA_RxConfig()中配置SD-DMA2接收
		   6) 在SD_DMA_TxConfig()中配置SD-DMA2发送
		   7) 在sdcard.c中配置宏定义BLOCK_SIZE和NUMBER_OF_BLOCKS

		   SD驱动程序使用步骤
		   1) 配置好驱动的设置项
		   2) 调用SD_NVIC_Configuration()进行中断初始化
		   3) 设置SDIO_IRQHandler()在合适的地方确保进入中断
		   4) 在main函数中调用SD_Test()即可完成SD卡配置
		   5) 调用SD_Single(Multi)Write(Read)(...)即可完成读写

 		   SD驱动程序现象
		   1) 调用SD_Test()之后打开串口设置波特率115200即可看到SD卡的信息

		   条件编译配置开关
		   1) 如需要使用SD测试函数请宏定义SD_TEST_MODE_OPEN 
		   2) 如需要使用SD DMA传输请宏定义SD_DMA2_MODE_OPEN

		   ROM Size = Code + RO-data +RW-data
		   RAM Size = RW-data + ZI-data
  		   Program Size: Code=20064 RO-data=2160 RW-data=468 ZI-data=1284  
...
#define BLOCK_SIZE            512 /* 每个Block的字节数,必须为512否则错误 */
#define NUMBER_OF_BLOCKS      32  /* 多次读写Block的数 */
#define MULTI_BUFFER_SIZE     (BLOCK_SIZE * NUMBER_OF_BLOCKS)//缓冲区大小
//#define SD_TEST_MODE_OPEN
#define SD_DMA2_MODE_OPEN														
/*----------------------------------------------
					变量声明
-----------------------------------------------*/
...
volatile  uint32_t StopCondition = 0; 							//用于停止卡操作的标志
volatile  SD_Error TransferError = SD_OK; 						//用于存储卡错误，初始化为正常状态
volatile  uint32_t TransferEnd = 0;	   							//用于标志传输是否结束，在中断服务函数中调用

#ifdef SD_TEST_MODE_OPEN
...
{		   
	...
	{    
 	...
 	printf( " \r\n 内存卡类型: %d ", SDCardInfo.CardType );
	printf( " \r\n 内存卡容量: %dMB ", SDCardInfo.CardCapacity/1000000);
	printf( " \r\n 内存块大小: %d字节", SDCardInfo.CardBlockSize );
...
/*----------------------------------------------
 函数功能: 为SDIO接收数据配置DMA2的通道4的请求
 函数参数: BufferDST：用于装载数据的变量指针
 		   BufferSize：	缓冲区大小
 返 回 值: 无
-----------------------------------------------*/
#ifdef SD_DMA2_MODE_OPEN
...
/*----------------------------------------------
 函数功能: 向SD卡写入一个块的数据
 函数参数: writebuff写入缓冲区的指针
 		   WriteAddr写入SD卡的地址
 调用示例: SD_SingleWrite("SD卡1",Add);
-----------------------------------------------*/
void SD_SingleWrite(uint8_t *writebuff, uint32_t WriteAddr)
{
	SD_WriteBlock(writebuff,WriteAddr*BLOCK_SIZE,BLOCK_SIZE);
...
/*----------------------------------------------
 函数功能: 从SD卡写读一个块的数据
 函数参数: readbuff读取缓冲区的指针
 		   ReadAddr读取SD卡的地址
 调用示例: SD_SingleRead(buffer,Add);
-----------------------------------------------*/
void SD_SingleRead(uint8_t *readbuff, uint32_t ReadAddr)
{
	SD_ReadBlock(readbuff,ReadAddr*BLOCK_SIZE,BLOCK_SIZE);
...
/*----------------------------------------------
 函数功能: 从SD卡写多个块的数据
 函数参数: writbuff写缓冲区的指针
 		   WriteAddr写SD卡的地址
		   NumberOfBlocks多个块的数量
-----------------------------------------------*/
void SD_MultiWrite(uint8_t *writebuff, uint32_t WriteAddr, uint32_t NumberOfBlocks)
{
	SD_WriteMultiBlocks(writebuff,WriteAddr*BLOCK_SIZE,BLOCK_SIZE,NumberOfBlocks);
...
/*----------------------------------------------
 ...
 函数参数: readbuff读取缓冲区的指针
		   ...
		   NumberOfBlocks多个块的数量
-----------------------------------------------*/
void SD_MultiRead(uint8_t writebuff[], uint32_t WriteAddr, uint32_t NumberOfBlocks)
{
	SD_ReadMultiBlocks(writebuff,WriteAddr*BLOCK_SIZE,BLOCK_SIZE,NumberOfBlocks);
    SD_WaitReadOperation();
	delayms(1);
//    while(SD_GetStatus() != SD_TRANSFER_OK); 	
}
...
/*----------------------------------------------
 ...
 函数参数: pBuffer1,pBuffer2 要比较的缓冲区的指针
 		   BufferLength      缓冲区长度
 返 回 值: PASSED 相等
 		   FAILED 不等
-----------------------------------------------*/
...
/*----------------------------------------------
 函数功能: 检查缓冲区的数据是否为0
 函数参数: pBuffer 要比较的缓冲区
 		   BufferLength 缓冲区长度
 返 回 值: PASSED 缓冲区的数据全为0
